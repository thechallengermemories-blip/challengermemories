    ...
    >
      ...
      >
        {[...Array(40)].map((_, i) => (
          <div
            key={i}
            className="absolute bg-white rounded-full opacity-30 blur-[1px]"
            style={{
              top: `${Math.random() * 100}%`,
      ...
      {/* LAYER 3: NASA-STYLE HUD (Updated for Challenger) */}
      ...
      {/* LAYER 4: MAIN CONTENT (Restored "Eternal Journey") */}
      <motion.div
      ...
      >
        ...
        >
          Their footsteps remain in the stars. Join us in honoring the crew of 
          <span className="text-white font-normal block mt-1">Space Shuttle Challenger</span>
        </motion.p>
        ...
        >
          {/* ACTION: Navigate to /share-story */}
          <button 
            onClick={() => router.push("/share-story")}
            className="group relative w-full sm:w-auto px-10 py-4 bg-white rounded-full overflow-hidden transition-transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2"
          >
          ...
          <button 
            ...
            className="w-full sm:w-auto px-10 py-4 rounded-full border border-white/20 bg-black/20 backdrop-blur-md text-white hover:bg-white/10 transition-all uppercase tracking-widest text-xs font-medium"
          >
            Explore Legacy
          </button>
        </motion.div>
